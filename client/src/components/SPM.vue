  ...
  <div>
    <v-navigation-drawer v-model="drawer" class="yellow lighten-4" app>
      <div style="margin: 15px;">
        <v-select
            v-model="stand_filter_values"
            :items="stand_filter_items"
            label="Stand"
            multiple
        ...
        <v-select
            v-model="airline_filter_values"
            :items="airline_filter_items"
            label="Airline"
            multiple
            @change="filtered = true"
        ></v-select>
        ...
        <v-range-slider
            ...
            color="#616161"
            track-color="#EEEEEE"
            v-model="at_filter_values"
            label="AT"
            :max="at_filter_items.max"
            :min="at_filter_items.min"
            thumb-label
            :thumb-size="20"
            @change="filtered = true"
        ></v-range-slider>
        ...
        <v-range-slider
            ...
            track-color="#EEEEEE"
            label="GT"
            v-model="gt_filter_values"
            :max="gt_filter_items.max"
            :min="gt_filter_items.min"
            thumb-label
            :thumb-size="20"
            @change="filtered = true"
        ></v-range-slider>
        ...
        <v-range-slider
            ...
            track-color="#EEEEEE"
            label="RH"
            v-model="humid_filter_values"
            :max="humid_filter_items.max"
            :min="humid_filter_items.min"
            thumb-label
            :thumb-size="20"
            @change="filtered = true"
        ></v-range-slider>
        <v-autocomplete v-model="event_filters_selection"
                        :items="event_filters.filter(function(d) {
                          fis = d.text.includes('-')
                          return mode===0?!fis:fis;} )"
                        label="Events (type to filter...)"
                        clearable
                        dense
                        hint="sequences with specific events"
                        persistent-hint
                        multiple
                        @change="filtered = true">
        </v-autocomplete>
    ...
    <v-container fluid>
          ...
          <div>
            <v-layout align-center justify-center>
              <div class="text-xs-center">
                <v-btn x-small color="yellow lighten-3" @click.stop="drawer = !drawer" disabled>
                  Advanced filtering (before mining)
                </v-btn>
                <v-btn x-small color="indigo lighten-4" @click="getPatterns" disabled>
                  Mine
                </v-btn>
            ...
            <v-col>
              <div id="table_legend">
                <table class="table-legend" v-show="mode === 0">
                  <thead>
                  <tr>
                    <th colspan="3">Antecedent</th>
                    <th colspan="3">Consequent</th>
                  </tr>
                  </thead>
                  <tbody>
                  <tr>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="6" fill="#B3E5FC"/>
                      </svg>
                    </td>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="8" fill="#29B6F6"/>
                      </svg>
                    </td>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="10" fill="#01579B"/>
                      </svg>
                    </td>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="6" fill="#FFCCBC"/>
                      </svg>
                    </td>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="8" fill="#FF8A65"/>
                      </svg>
                    </td>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="10" fill="#BF360C"/>
                      </svg>
                    </td>
                  </tr>
                  <tr>
                ...
                <table class="table-legend" v-show="mode === 1">
                  ...
                  <tr>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="6" fill="#E1BEE7"/>
                      </svg>
                    </td>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="8" fill="#BA68C8"/>
                      </svg>
                    </td>
                    <td>
                      <svg width="20" height="20">
                        <circle cx="10" cy="10" r="10" fill="#4A148C"/>
                      </svg>
                    </td>
                  </tr>
                  <tr>
              ...
              <v-dialog v-model="matrix_dialog" width="500">
                ...
                <v-card>
                  ...
                  <v-card-text>
                    ...
                    <b>Columns:</b> turnaround events.<br/>
                    <b>Rows:</b> patterns.<br/>
                    <b>Circles:</b> event in the pattern, color and size represent both class of
                    event (early, on-time, delayed) and role of event in the pattern (antecedent or
                    consequent).<br/>
                    ...
                    * The <b>support</b> and <b>confidence</b> columns are displayed in <b>fractions</b>.
                    ...
                    <b>Columns:</b> Deltas between pairs of events.<br/>
                    <b>Rows:</b> patterns.<br/>
                    <b>Circles:</b> deltas in the pattern, color and size represent class of
                    delta (short, on-time, long).<br/>
                    * Circles in each row represent co-occurring deltas of certain class in the
                    records.
                  </v-card-text>
                  <v-card-actions>
                    <v-spacer></v-spacer>
      ...
      <v-row class="border rounded m-1 p-1" dense>
        ...
        <v-col md="4">
          <Plotly
              :data="heatmap_series"
              :layout="heatmap_options"
              :display-mode-bar="false"
          ></Plotly>
        </v-col>
        <v-col md="4">
          <Plotly :data="pc_data" :layout="pc_layout" :display-mode-bar="false"></Plotly>
        </v-col>
      </v-row>
      <v-row>
        <v-col>
          <v-card-text class="lighten-4 text-center">
            <v-btn x-small color="cyan lighten-4" @click="pt_show = !pt_show">
      ...
      <v-row>
        <v-col>
          ...
          <p v-show="pt_show">
            <v-icon color="red">mdi-lightbulb-on-outline</v-icon>
            Details are reduced and anonymized due to NDA restrictions.
          </p>
        </v-col>
      </v-row>
    </v-container>
      ...
      <v-row>
        <v-card color="transparent" flat dark>
          <v-card-text>
            ...
            <!--            <v-btn-->
            <!--                :color="overlay_message.length===0?'transparent':'white'"-->
            <!--                icon-->
            <!--                @click="refresh"-->
            <!--            >-->
...
export default {
  ...
  data() {
    return {
      ...
      humid_filter_values: [],
      event_filters: [],
      event_filters_selection: [],
      rule_options: {support: 10, confidence: 70, window: 15},
      overview: [],
      showLables: false,
      color_choice: "none",
      ...
      heatmap_options: {
        ...
        yaxis: {"categoryorder": "array", 'categoryarray': ['night', 'evening', 'afternoon', 'morning']}
      },
      sunburst_data: [
        {
          type: "sunburst",
  ...
  methods: {
    refresh() {
      this.$router.push({name: "SPM"});
    },
    clearSelections() {
      this.selected_patterns_from_table = [];
      this.selected_patterns_from_dag = [];
      this.related_rule_ids = [];
    },
    getRuleConfigRequest() {
      let request = {config: this.rule_options, mode: this.mode, data: 'airport'};
      if (this.filtered === true) {
        request["filter"] = {
          airline: this.airline_filter_values,
          stand: this.stand_filter_values,
          at: this.at_filter_values,
          gt: this.gt_filter_values,
          humid: this.humid_filter_values,
          events: this.event_filters_selection
        };
      }
      return request;
    },
      ...
      axios
          ...
          .then(res => {
            if (res.data.toomany === "0") {
              ...
              this.heatmaps_list = res.data.time_dist_heatmaps;
              this.sunbursts_list = res.data.sunbursts;
              this.pc_list = res.data.pc;
              this.count = res.data.count;
              this.clearSelections();
              this.generateDashboard();
            } else {
              window.alert("Oops! Number of patterns too high to process :( \n Please try increasing the support\\confidence.")
            }
          })
          .catch(error => {
            console.error(error);
    ...
    resetViews(matrix_index = 0, update_table = true) {
      ...
      this.heatmap_series[0].x = this.heatmaps_list[matrix_index].x;
      this.heatmap_series[0].y = this.heatmaps_list[matrix_index].y;
      this.heatmap_series[0].z = this.heatmaps_list[matrix_index].z;
      ...
      this.pc_data[0].dimensions = this.pc_list[matrix_index];
      this.headers = [];
      this.rows = [];
    },
    ...
    generateDAGOverview() {
      ...
      function nodeSelected(event, d) {
        ...
        vm.selected_patterns_from_table = []; //clear selection, avoid problems for showing views
      ...
      for (let i = 0; i < matrix_rows.length; i++) {
        ...
        for (let j = 0; j < matrix_columns.length; j++) {
          ...
          if (matrix_columns[j] === "support" || matrix_columns[j] === "confidence") {
            ...
            for (let z = 0; z < stats[matrix_columns[j]].length; z++) {
              if (stats[matrix_columns[j]][z].tag === val) {
    ...
    updateRuleTable(matrix_columns, matrix_rows) {
      ...
      function itemCircle(selection) {
        ...
        selection
            ...
            .attr("cy", center)
            .attr("r", function (d) {
              return vm.radiusScaler(d.v, unit_radius);
            })
      ...
      rule_rows.on("click", function (event, d) {
        if (vm.selected_patterns_from_table.includes(d.rid)) {
          let index = vm.selected_patterns_from_table.indexOf(d.rid);
          if (index >= 0) {
            vm.selected_patterns_from_table.splice(index, 1);
          }
          d3.select(this).classed("row-selected", false);
        } else {
          vm.selected_patterns_from_table.push(d.rid);
    ...
    radiusScaler(label, radius) {
      if (label.includes("E") || label.includes("S")) return radius * 0.7;
      else if (label.includes("O")) return radius * 0.85;
      return radius;
    },
    updatePanelsBySelection() {
      let backend_request_ids;

      if (this.selected_patterns_from_table.length === 0) {
        if (this.related_rule_ids.length > 0) {
          //views must be related to selected node from overview graph
          backend_request_ids = this.related_rule_ids;
        } else {
          this.resetViews(this.current_matrix, false);
          return;
        }
      } else {
        //views must be related to selected patterns from table
        backend_request_ids = this.selected_patterns_from_table;
      }
      const path = this.url + "distribution_data";
      this.overlay = true;
      let request = this.getRuleConfigRequest();
      request["rids"] = backend_request_ids;
      axios
          .post(path, request)
          .then(res => {
            this.heatmap_series[0].x = res.data.heatmap.x;
            this.heatmap_series[0].y = res.data.heatmap.y;
            this.heatmap_series[0].z = res.data.heatmap.z;
            let sunburst = res.data.sunburst;
            this.sunburst_data[0].labels = sunburst.labels;
            this.sunburst_data[0].ids = sunburst.ids;
            this.sunburst_data[0].values = sunburst.values;
            this.sunburst_data[0].parents = sunburst.parents;
            this.pc_data[0].dimensions = res.data.pc;
            this.headers = res.data.performance_columns;
            this.rows = res.data.performance_rows;
          })
          .catch(error => {
            console.error(error);
          })
    ...
    setUpFilterForm() {
      ...
      axios
          .get(path)
          .then(res => {
            this.stand_filter_items = res.data.stand;
            this.stand_filter_values = res.data.stand;
            this.airline_filter_items = res.data.airline;
            this.airline_filter_values = res.data.airline;
            this.at_filter_items = res.data.at;
            this.at_filter_values = [res.data.at.min, res.data.at.max];
            this.gt_filter_items = res.data.gt;
            this.gt_filter_values = [res.data.gt.min, res.data.gt.max];
            this.humid_filter_items = res.data.humid;
            this.humid_filter_values = [res.data.humid.min, res.data.humid.max];
            this.event_filters = res.data.events;
          })
          .catch(error => {
            console.error(error);
          });
